...
def test_all_classes():
    ...
    data = data_dir['data_all_atom'][::1000]
    dihedrals = data_dir['dihedrals'][::1000]
    
    # Example input tensor of shape [batch_size, 3N]
    batch_size = data.shape[0]
    N = data.shape[1] // 3 # Number of atoms
    example_input = torch.tensor(data.reshape(batch_size, 3 * N), dtype=torch.float32)

    # Test GramMatrixLayer
    selected_rows = [i for i in range(N)]  # Example: select the 1st and 3rd rows
    mass_matrix = torch.zeros(len(selected_rows), N)
    for i, row in enumerate(selected_rows):
        mass_matrix[i, row] = 1

    gram_matrix_layer = GramMatrixLayer(mass_matrix)
    output_gram_matrices = gram_matrix_layer(example_input)
    print("Gram Matrix Layer Output:")
    print(output_gram_matrices.shape)
    return None 
    # Check gradients
    example_input.requires_grad_(True)
    # gradcheck(gram_matrix_layer, (example_input,), eps=1e-4, atol=1e-2)

    # Test DihedralAngleLayer
    selected_atoms = [3, 6, 9, 13]  # Example: select the first 4 atoms
    mask_matrix = torch.zeros(len(selected_atoms), N)
    for i, atom in enumerate(selected_atoms):
        mask_matrix[i, atom] = 1

    dihedral_angle_layer = DihedralAngleLayer(mask_matrix)
    output_angles = dihedral_angle_layer(example_input)
    print("Dihedral Angle Layer Output:")
    print(output_angles)

    # Check gradients
    # gradcheck(dihedral_angle_layer, (example_input,), eps=1e-4, atol=1e-2)

    # Test RecenterBondLayer
    atom_ind = [1, 2]  # Example: use the 2nd and 3rd atoms for recentering
    recenter_bond_layer = RecenterBondLayer(atom_ind, batch_mode=True)
    output_recentered = recenter_bond_layer(example_input)
    print("Recenter Bond Layer Output:")
    print(output_recentered)

    # Check gradients
    # gradcheck(recenter_bond_layer, (example_input,), eps=1e-4, atol=1e-2)

    # Test OrthogonalChangeOfBasisBatched
    orthogonal_change_of_basis_layer = OrthogonalChangeOfBasisBatched()
    output_transformed = orthogonal_change_of_basis_layer(example_input)
    print("Orthogonal Change of Basis Batched Layer Output:")
    print(output_transformed)

    # Check gradients
    # gradcheck(orthogonal_change_of_basis_layer, (example_input,), eps=1e-4, atol=1e-2)
